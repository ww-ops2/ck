...
foreach ($relPath in $filesToPush) {
    ...
    $cloudSha = $null
    try {
        $resp = Invoke-RestMethod -Uri "$apiBase/contents/3-库存管理/$relPath" -Headers $headers -Method Get -ErrorAction Stop
        $cloudSha = $resp.sha
    } catch {}

    $msg = "v5.42 " + $(if ($cloudSha) { "update" } else { "add" }) + " 3-库存管理/$relPath"
    $body = @{
        message = $msg
        content = $b64
    }
    ...
    try {
        $response = Invoke-RestMethod `
            -Uri "$apiBase/contents/3-库存管理/$relPath" `
            -Headers $headers `
            -Method Put `
            -Body $bodyJson `
